...
function runHook(platform, config) {
  ...
  // Legacy env values may point at the node launcher (rust-skills.js);
  // run those through node instead of exec'ing a .js file directly.
  const viaNode = binary.endsWith(".js");
  const command = viaNode ? process.execPath : binary;
  const args = viaNode ? [binary, "hook", platform] : ["hook", platform];
